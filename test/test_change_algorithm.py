...
class TestAlgorithms(unittest.TestCase):
    ...
    def _run_non_canonical_coins(self, algorithm: ChangeAlgorithm):
        ...
        test_cases = [
            ...
            ({1: 1, 3: 1, 5: 1, 9: 1, 17: 1, 33: 1, 65: 1, 129: 1},
                {1: 1, 3: 1, 5: 1, 9: 1, 17: 1, 33: 1, 65: 1, 129: 1}, 262)
        ]
    ...
    def _run_canonical_coins(self, algorithm: ChangeAlgorithm):
        test_cases = [
            ...
            ({200: 1, 100: 1, 50: 1, 20: 1, 10: 1, 5: 1, 2: 1, 1: 1},
                {200: 10, 100: 10, 50: 10, 20: 10, 10: 10, 5: 10, 2: 10, 1: 10}, 388)
        ]
        for test_case in test_cases:
            with self.subTest(f"coins: {test_case[1]}, amount: {test_case[2]}"):
                self.assertEqual(
                    test_case[0], algorithm.calculate_coins(algorithm.sort_coins(test_case[1]), test_case[2]))
        ...
        for test_case in exception_test_cases:
            with self.subTest(f"coins: {test_case[0]}, amount: {test_case[1]}"):
                self.assertRaises(
                    CalculationError, algorithm.calculate_coins, algorithm.sort_coins(test_case[0]), test_case[1])
